      ...
      <div className="flex flex-col gap-6">
        {entries.map((entry) => (
          <div key={entry.id} className="relative flex gap-4">
            {/* Timeline dot */}
            <div className="relative z-10 mt-1.5 h-[15px] w-[15px] shrink-0 rounded-full border-2 border-[var(--solvyn-olive)] bg-[var(--solvyn-bg-base)]" />

            {/* Card */}
            <div className="flex-1 rounded-lg border border-[var(--solvyn-border-subtle)] bg-[var(--solvyn-bg-elevated)] p-4">
              ...
              <p className="mt-2 whitespace-pre-wrap text-[13px] leading-relaxed text-[var(--solvyn-text-secondary)]">
                {entry.description}
              </p>
            </div>
          </div>
